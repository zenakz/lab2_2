...
class SimilarityFinderStateTest {
    ...
    @Test public void calculateNullSets() {
        similarityFinder = new SimilarityFinder((elem, seq) -> SearchResult.builder().withFound(false).build());

        assertThrows(NullPointerException.class, () -> similarityFinder.calculateJackardSimilarity(seq1, seq2));
    }

    @Test public void calculateEmptySets() {
        seq1 = new int[] {};
        seq2 = new int[] {};
        similarityFinder = new SimilarityFinder((elem, seq) -> SearchResult.builder().withFound(false).build());
        double result = similarityFinder.calculateJackardSimilarity(seq1, seq2);
